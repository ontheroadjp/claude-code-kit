...
SAMPLE_COMMANDS_LOG_CONTENT = textwrap.dedent(
    """\
    ...
    assert routine_ops["patterns_needing_approval"] == [
        {
            ...
            "blocked_count": 0,
            "sample_commands": [
                {"command": 'git commit -m "later"', "count": 1},
                {"command": 'git commit -m "wip"', "count": 1},
            ],
        }
    ]
...
def test_pattern_command_samples_orders_by_frequency_then_alphabetically(
    ...
    monkeypatch.setattr(analyze_common, "repo_root", lambda: tmp_path)
    write_log(tmp_path, "2026-08", SAMPLE_COMMANDS_LOG_CONTENT)
    ...
    assert patterns["git push"]["sample_commands"] == [
        {"command": "git push origin feature-a", "count": 2},
        {"command": "git push origin feature-b", "count": 1},
    ]
