...
│   ├── test-task-manager.sh       ← task-manager orchestration contract
│   └── test-git-pr-merge.sh       ← reviewed PR delivery contract
├── install/
...
`test-mtg.sh` はagenda routing、ユーザー主導の close、`/new-issue` の明示指示ゲート、Facts / Assessment / Opinions / Proposals による詳細検討を検証する。`test-coding-guidelines.sh` はReact/Next.js layerの依存順・routing・repository非依存性、`test-workflow-contracts.sh` は docs-sync/init-docs/task/git-pr 間の契約、`test-work-multi.sh` は worktree isolation、`test-task-manager.sh` はbatch orchestration、`test-git-pr-merge.sh` はapproved-head drift・latest-main refresh・validation・workspace safety・squash deliveryを固定文字列で検証する。
