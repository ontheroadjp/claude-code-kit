...
# Batch review, ordered squash delivery, and automatic docs delivery.
assert_contains "$TASK_MANAGER" 'complete Draft PR setを再提示' 'Draft rejection loops over the complete set'
assert_contains "$TASK_MANAGER" '承認された全source PR番号、approved scope、behavior' 'batch approval fixes the reviewed PR set and scope'
assert_contains "$TASK_MANAGER" 'source PRを入力順に1本ずつ処理' 'source PRs merge in input order'
assert_contains "$TASK_MANAGER" '最初のPRをdeliveryしてから次のPR' 'the first PR is delivered before later integration starts'
assert_contains "$TASK_MANAGER" '2本目以降では' 'only later PRs incorporate preceding merged work'
...
assert_contains "$TASK_MANAGER" 'actual branch上で解消' 'conflicts are resolved on the actual branch'
assert_contains "$TASK_MANAGER" 'focusedしたlocal test' 'conflict repairs run focused tests'
...
assert_contains "$TASK_MANAGER" 'planned local delivery validation' 'missing CI coverage has a local delivery fallback'
assert_contains "$TASK_MANAGER" 'equivalent required GitHub CI' 'required CI is authoritative when available'
assert_contains "$TASK_MANAGER" 'missing、skipped、対応不明のvalidationをpassとして扱わない' 'missing validation is never treated as passing'
assert_contains "$TASK_MANAGER" 'current actual PR branchに対するchecksの完了を待ち' 'required checks validate the current actual branch'
assert_contains "$TASK_MANAGER" 'actual branchがcurrent latest `origin/main`を含むこと' 'later branches are rechecked against latest main before merge'
assert_contains "$TASK_MANAGER" 'SHAをexpected stateとして記録・比較するretry guardは設けない' 'SHA-based retry guards are prohibited'
assert_absent "$TASK_MANAGER" 'expected head SHA' 'source delivery does not track expected head SHAs'
assert_absent "$TASK_MANAGER" 'expected headに対するrequired GitHub checks' 'required checks do not use an expected-head transaction'
assert_absent "$TASK_MANAGER" 'latest main SHAがStep 1で記録した値' 'latest-main verification does not compare a recorded SHA'
assert_absent "$TASK_MANAGER" 'current remote headを取得してexpected headを更新' 'repair pushes do not update expected-head state'
assert_contains "$TASK_MANAGER" '対象PRだけをReady化' 'only the next source PR becomes ready'
assert_contains "$TASK_MANAGER" '`gh pr merge --squash`' 'every source PR uses explicit squash merge'
