...
assert_contains "$PR_REVIEW" '--dangerously-bypass-approvals-and-sandbox' 'the Codex reviewer runs without the OS sandbox (bwrap init is unreliable across hosts)'
assert_absent "$PR_REVIEW" '--sandbox workspace-write' 'the Codex reviewer does not rely on the workspace-write sandbox'
assert_absent "$PR_REVIEW" 'sandbox_workspace_write.network_access' 'the Codex reviewer does not rely on sandbox network_access config'
