...
assert_contains "$COMMAND" 'predecessor_approved_head: <chain position k≥2' 'delegated context accepts the predecessor approved head'
assert_contains "$COMMAND" '**delegated chain 例外**' 'a chain exception relaxes head/base equality for frozen-base evidence'
assert_contains "$COMMAND" 'known-empty delivery merge' 'the chain exception only spans tree-neutral delivery merges'
assert_contains "$COMMAND" 'sibling-worker divergence 由来のconflictは構造的に発生せず' 'chain delivery has no sibling-divergence conflict by construction'
