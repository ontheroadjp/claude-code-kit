...
assert_contains "$COMMAND" '`validation_scope` が exact `full`' 'reuse requires full validation scope'
assert_contains "$COMMAND" '`validation_plan` が approved `final_validation_plan` と完全一致' 'reuse requires the exact approved plan'
assert_contains "$COMMAND" '`validated_head_sha` が current post-refresh remote head SHA と完全一致' 'reuse is bound to the current head SHA'
assert_contains "$COMMAND" '`validated_base_sha` が Phase 2 で記録した current latest `origin/main` SHA と完全一致' 'reuse is bound to the current base SHA'
assert_contains "$COMMAND" 'absent、targeted-only、failed、incomplete' 'missing or insufficient evidence falls back to validation'
assert_contains "$COMMAND" '同じ full-suite local commands は再実行せず' 'matching evidence skips duplicate local full-suite execution'
assert_contains "$COMMAND" 'required checks は evidence で省略せず' 'required checks remain authoritative'
assert_absent "$COMMAND" 'predecessor_approved_head' 'no predecessor-approved-head delegated field remains'
assert_absent "$COMMAND" 'delegated chain 例外' 'the delegated chain evidence exception is removed'
assert_absent "$COMMAND" 'known-empty delivery merge' 'no known-empty delivery merge recording remains'
assert_contains "$COMMAND" 'owned_worktree: <permission to create an isolated delivery worktree for this PR>' 'delegated delivery always creates its own worktree'
assert_contains "$COMMAND" 'delegated deliveryでは対象PRが起動時点のlatest `origin/main`から分岐している' 'delegated delivery branches from launch-time latest origin/main'
