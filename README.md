...
| `/task-manager` | Independent batch workflow for one to three implementation issues. After one combined plan approval, real `task-worker` sub-agents create isolated Draft source PRs; one approval of the complete Draft set authorizes ordered source merges and one automatically merged localized documentation PR. Equivalent integration conflict resolutions are replayed during later PR branch refreshes without skipping refresh or validation. |
