...
- GitHub: `gh label list`, `gh label create`, `gh issue create`, `gh issue comment`（親子issue化時の相互参照コメント）
...
- 親子issue化時の `gh issue comment`（相互参照コメント）は `tool:gh_issue_write` のスコープ対象外（対象 issue 番号ごとの numbered grant が必要なため）で、standalone 起動でも通常の確認プロンプトに従う
