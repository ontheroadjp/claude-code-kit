...
`SAMPLE_COMMANDS_LOG_CONTENT`（4行、`sample_commands` の頻度順ソート専用）: `git push origin feature-a`（user_prompt x2、異なるセッション）・`git push origin feature-b`（user_prompt x1）・`git push (session)`（approved x1、`patterns_needing_approval` には出ない件数として利用）。
...
- `pattern_command_samples()` が `sample_commands` を `(count 降順, detail 昇順)` で並べること（`SAMPLE_COMMANDS_LOG_CONTENT` の `git push origin feature-a`（count=2）が `git push origin feature-b`（count=1）より先に来る）
...
根拠: `tests/scripts/test_analyze_auto_approve.py:51-135`, `tests/scripts/test_analyze_auto_approve.py:153-167`
...
`SAMPLE_COMMANDS_LOG_CONTENT` を専用の合成ログとして分離したのは、`sample_commands` の並び順（頻度→辞書順のタイブレーク）という新しい振る舞いを、既存の `LOG_CONTENT`（詳細な件数アサーションを既に大量に持つ）に混ぜず独立に検証するため（issue #278）。issue #280 で truncate 自体が撤廃されたため、当初このフィクスチャに含まれていた120文字超の `git commit` detail（truncate フラグ検証専用、`LONG_COMMAND_DETAIL`）と対応するテストは削除した。
