...
| auto-approve候補レビュー | `commands/triage-issues-for-auto-approve.md` | `auto-approve-candidate` label 付き open issue を一覧化し、issue ごとにハザード分析を開示、yes/no ゲートを経て yes の場合のみ `/work #N` の実行を案内するスタンドアロン入口。read-only で `/work` は自身で呼ばない | `commands/triage-issues-for-auto-approve.md:1-91` |
...
- `auto-approve-candidate` issue のハザード分析を開示し、実装着手を人間の承認ゲート越しに `/work #N` へ案内する read-only 任意入口は `/triage-issues-for-auto-approve`。`/work` は自身で呼ばない。根拠: `commands/triage-issues-for-auto-approve.md:1-12`
