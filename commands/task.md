...
作業ブランチを新規作成または既存ブランチへ切り替えた直後、現在のブランチ名を取得し、Claude Code の会話スレッド名を `/rename <作業ブランチ名>` と同じ結果になるよう更新する。ブランチ名を推測・手入力してはならず、Git が返した現在のブランチ名を用いる。スレッド名の更新に失敗しても、Git のブランチ切替や以降の実装を中断してはならない。
