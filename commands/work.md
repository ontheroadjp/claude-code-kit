...
全ての作業のエントリポイントです。ゲート確認・ワークスペース管理・ルーティング判定を行い、`commands/task.md`（docs 変更あり）または `commands/patch.md`（docs 変更なし）を Read して委譲します。
...
`git branch` を実行し、main 以外のブランチが存在するか確認する。

存在する場合、各ブランチに対して `git branch -d <branch>` を試みる:
- 削除成功 → 続行
- 削除失敗（unmerged）→ 「未マージのブランチが存在します。手動でクリーンアップしてください」とユーザーに伝えて終了する

main のみになった状態で次のゲートへ進む。
