...
指定した PR のレビューコメントを取得し、各コメントに対して「対応する」または「対応しない（理由を返信）」をユーザーが選択できるコマンドです。
...
- PR が存在する場合: タイトルと URL を表示して Step 2 へ進む
...
- `/work` を呼び出し、コメント内容を作業指示として渡す
- `/work` の完了後、このコメントへ返信する:
  ```bash
  # インラインコメントへの返信
