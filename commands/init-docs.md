...
同名ブランチが既に存在する場合は、以下のいずれかを行う:
- そのブランチで作業を継続してよいことを確認できる場合: `git checkout docs/init-docs-<YYYYMMDD>` で切り替える
...
（作業ブランチ: docs/init-docs-<YYYYMMDD>）
```
...
現在ブランチが G-2 で作成・切り替えた `docs/init-docs-<YYYYMMDD>` であることを確認する。
main ブランチ上である場合は、commit・push・PR 作成を中止し、G-2 へ戻って作業ブランチへ切り替える。
