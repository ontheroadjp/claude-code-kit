...
- 環境変数 `GH_TOKEN` に GitHub token が設定され、`gh` コマンドがそのまま認証情報として使えること。呼び出し元がこのコマンドの**プロセス起動時の環境変数**として渡す（`gh` はコマンド内でのインライン prefix や `export` を必要としない）。未設定または空の場合は「GH_TOKEN が設定されていません」と報告して終了する
...
- `gh` コマンドは常にプレフィックス無しの `gh ...` で呼び出す（`GH_TOKEN=... gh ...` のようなインライン環境変数 prefix や、`export GH_TOKEN=...` の実行は行わない）。呼び出し元の tool allowlist（例: `Bash(gh pr view *)`）は「コマンドが `gh` で始まる」ことを前提にパターンマッチしており、インライン prefix を付けるとマッチせず拒否される。`export` は Bash tool 呼び出しごとに新しいシェルが使われる環境では後続の呼び出しに引き継がれない。いずれも実地確認で発見した
