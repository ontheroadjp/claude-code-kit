...
- 最小更新方針を確定する:
    ...
    - 手順更新（setup/run/test）
    - 仕様サマリ更新（specification_summary は該当箇所のみ。Phase 1 Step 2 で citation を取得済みの場合はその行範囲を `offset`/`limit` で対象読みし、独自の再特定は行わない。citation がない場合のみ Glob/Grep 等で該当箇所を特定する）
...
- **確認必要**: git diff の値をそのまま転記するのではなく、文脈・意図を解釈して文章化する更新
    - 仕様サマリの説明文（プローズ）を新しい振る舞いに合わせて書き換える場合
    - `pr-body.md` の「設計意図・背景」等の補助情報を解釈して docs に反映する場合
    - 更新対象 docs の特定や根拠付けに複数の解釈がありうる場合
...
- 1項目でも「確認必要」に該当する場合:
    - 該当項目について、反映する文章そのものではなく、その根拠となった解釈・理解を提示する（例:「この変更は〇〇という意図だと解釈し、それを△△という形で仕様サマリに反映する予定」）
...
- (B) citation がなく、かつ独自探索でも specification_summary.md の「該当箇所」が特定できない（全体書換えしか手がない状態）
