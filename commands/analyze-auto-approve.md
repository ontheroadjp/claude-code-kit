...
- `routine_ops.patterns_needing_approval`（user_prompt に落ちている定型処理パターンの上位リスト。各要素はパターン名・user_prompt件数・approved件数・blocked件数に加え、`sample_commands`（そのパターンで実際に user_prompt に落ちたユニークなコマンド文字列を頻度降順で最大10件）を持つ。`routine_ops` は `hooks/auto-approve-readonly.sh` の `check_session_approved()` が認識する git/gh write系コマンド形状で分類したもの）
